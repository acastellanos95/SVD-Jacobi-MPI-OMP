...
//        std::cout << "p: " << p_point << ", q:" << q_point << ", p_transform: " << p_point_to_local_index << ", q_transform: " << q_point_to_local_index << '\n';
          ...
          if(std::abs(alpha) > TOLERANCE){
            ...
            if (jobv == AllVec || jobv == SomeVec) {
              for (size_t i = 0; i < n; ++i) {
                tmp_p = c_schur * V.elements[iteratorC(i, p_point, ldv)] - s_schur * V.elements[iteratorC(i, q_point, ldv)];
                tmp_q = s_schur * V.elements[iteratorC(i, p_point, ldv)] + c_schur * V.elements[iteratorC(i, q_point, ldv)];
                V.elements[iteratorC(i, p_point, ldv)] = tmp_p;
                V.elements[iteratorC(i, q_point, ldv)] = tmp_q;
              }
            }
          }
        }
...
//        std::cout << "p: " << p_point << ", q:" << q_point << ", p_transform: " << p_point_to_local_index << ", q_transform: " << q_point_to_local_index << '\n';
          ...
          if(std::abs(alpha) > TOLERANCE){
            ...
            if (jobv == AllVec || jobv == SomeVec) {
              for (size_t i = 0; i < n; ++i) {
                tmp_p = c_schur * V_local.elements[iteratorC(i, p_point_to_local_index, ldv)] - s_schur * V_local.elements[iteratorC(i, q_point_to_local_index, ldv)];
                tmp_q = s_schur * V_local.elements[iteratorC(i, p_point_to_local_index, ldv)] + c_schur * V_local.elements[iteratorC(i, q_point_to_local_index, ldv)];
                V_local.elements[iteratorC(i, p_point_to_local_index, ldv)] = tmp_p;
                V_local.elements[iteratorC(i, q_point_to_local_index, ldv)] = tmp_q;
              }
            }
          }
        }
...
//      MatrixMPI A_local(m, local_set_columns.size());
      ...
      if(rank == ROOT_RANK){
        MPI_Request* requests = new MPI_Request[2*root_index_column_to_rank.size()];
        MPI_Status* statuses = new MPI_Status[2*root_index_column_to_rank.size()];
        ...
        for(auto column_index: root_index_column_to_rank){
          ...
          /* ------------------------------------- Send column of V -------------------------------------------------*/
          auto return_status_v = MPI_Isend(V.elements + (n * column_index.first), n, MPI_DOUBLE, column_index.second, n + column_index.first, MPI_COMM_WORLD, &requests[index_request++]);
          if(return_status_v != MPI_SUCCESS){
            std::cout << "problem on MPI_ISend on rank: " << rank << ", return status" << return_status_v << "\n";
          }
          std::cout << "sent column: " << column_index.first << " to rank: " << column_index.second << '\n';
        }

        auto return_status = MPI_Waitall(2 * root_index_column_to_rank.size(), requests, statuses);
        if(return_status != MPI_SUCCESS){
        ...
        */
        ...
        MPI_Request* requests = new MPI_Request[4*local_points.size()];
        MPI_Status* statuses = new MPI_Status[4*local_points.size()];
        size_t index_request = 0;

        for(auto &point: local_points){
          MatrixMPI p_vector(1, m), q_vector(1, m), v_p_vector(1, m), v_q_vector(1, m);
          MPI_Status status;
          ...
          /* ------------------------------------- Receive column p and q of V -------------------------------------------------*/
          return_status = MPI_Recv(v_p_vector.elements, m, MPI_DOUBLE, 0, n + p_index, MPI_COMM_WORLD, &status);
          if(return_status != MPI_SUCCESS){
            std::cout << "problem on MPI_Recv on rank: " << rank << ", return status" << return_status << "\n";
          }
          return_status = MPI_Recv(v_q_vector.elements, m, MPI_DOUBLE, 0, n + q_index, MPI_COMM_WORLD, &status);
          if(return_status != MPI_SUCCESS){
            std::cout << "problem on MPI_Recv on rank: " << rank << ", return status" << return_status << "\n";
          }
          ...
          */
          ...
          if(std::abs(alpha) > TOLERANCE){
            ...
            if (jobv == AllVec || jobv == SomeVec) {
              for (size_t i = 0; i < n; ++i) {
                tmp_p = c_schur * v_p_vector.elements[i] - s_schur * v_q_vector.elements[i];
                tmp_q = s_schur * v_p_vector.elements[i] + c_schur * v_q_vector.elements[i];
                v_p_vector.elements[i] = tmp_p;
                v_q_vector.elements[i] = tmp_q;
              }
            }
          }

          /* ------------------------------------- Send solved columns of A -------------------------------------------------*/
          ...
          /* ------------------------------------- Send solved columns of V -------------------------------------------------*/

          std::cout << "sending column: " << p_index << " to rank: " << 0 << '\n';
          return_status = MPI_Isend(v_p_vector.elements, n, MPI_DOUBLE, 0, n + p_index, MPI_COMM_WORLD, &requests[index_request++]);
          if(return_status != MPI_SUCCESS){
            std::cout << "problem on MPI_ISend on rank: " << rank << ", return status" << return_status << "\n";
          }
          std::cout << "sent column: " << p_index << " to rank: " << 0 << '\n';
          ...
          std::cout << "sending column: " << q_index << " to rank: " << 0 << '\n';
          return_status = MPI_Isend(v_q_vector.elements, n, MPI_DOUBLE, 0, n + q_index, MPI_COMM_WORLD, &requests[index_request++]);
          if(return_status != MPI_SUCCESS){
            std::cout << "problem on MPI_Send on rank: " << rank << ", return status" << return_status << "\n";
          }
          std::cout << "sent column: " << q_index << " to rank: " << 0 << '\n';
        ...
        auto return_status = MPI_Waitall(4*local_points.size(), requests, statuses);
        if(return_status != MPI_SUCCESS){
          std::cout << "problem on MPI_Waitall on rank: " << rank << ", return status" << return_status << "\n";
        }
      ...
      if(rank == ROOT_RANK){
        for(auto column_index: root_index_column_to_rank){
          MatrixMPI column_vector(1, m), v_column_vector(1, m);
          MPI_Status status;
          /* ----------------------------------- Gather A solutions ------------------------------------------------*/
          ...
          /* ----------------------------------- Gather V solutions ------------------------------------------------*/
          std::cout << "receiving column: " << column_index.first << " from rank: " << column_index.second << " in rank " << 0 << '\n';
          return_status = MPI_Recv(v_column_vector.elements, n, MPI_DOUBLE, column_index.second, n + column_index.first, MPI_COMM_WORLD, &status);
          if(return_status != MPI_SUCCESS){
            std::cout << "problem on MPI_Recv on rank: " << rank << ", return status" << return_status << "\n";
          }
          std::cout << "received column: " << column_index.first << " from rank: " << column_index.second << " in rank " << 0 << '\n';
          ...
          */
          ...
          #pragma omp parallel for
          for(size_t index_row = 0; index_row < m; ++index_row){
            A.elements[iteratorC(index_row, column_index.first, lda)] = column_vector.elements[index_row];
          }
          ...
          #pragma omp parallel for
          for(size_t index_row = 0; index_row < n; ++index_row){
            V.elements[iteratorC(index_row, column_index.first, ldv)] = v_column_vector.elements[index_row];
          }

          column_vector.free();
          v_column_vector.free();
        }
